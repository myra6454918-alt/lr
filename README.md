...
- **Огонь** (раздел «ОГОНЬ») — очаги разгораются, обугливают древесину (шейдер: коричневый → уголь с
  ...
  Персонаж загорается от пламени, коктейля и взрыва бочки: языки огня по низу экрана, оранжевый
  свет на стены, искры; горит ещё несколько секунд после выхода из огня (`C` — присесть и сбить пламя).
...
- **Карта** — здание 30×19 м, два этажа, атриум с мостиком, 2 внутренние деревянные и
  ...
  Внутри здания в каждой комнате есть укрытия на подходе от каждой двери. Пол ангара побит:
  выбоины с арматурой, воронки от взрывов с валом крошки и копотью, сколы и сетки трещин.
  На длинных стенах ангара — надпись **badVIno**.
- **Свет** — днём ангар светлый: заливка от остекления и отскок от бетона, лучи солнца сквозь
  проёмы кровли и косые лучи сквозь ленточное остекление. Без бликов.
- **Производительность** — число источников света постоянно (пулы: лампы, огонь, вспышки,
  прожекторы), заливка — сферическая гармоника вместо пяти направленных источников, реквизит
  запекается в общие меши, тени перерисовываются только когда что-то изменилось. Если кадр не
  укладывается в бюджет профиля, сначала выключается экранный AO, затем плавно снижается
  разрешение рендера; при запасе всё возвращается. `?fixedres` отключает адаптацию.
